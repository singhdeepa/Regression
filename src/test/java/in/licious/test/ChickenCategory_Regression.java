...
public class ChickenCategory_Regression extends BaseTest{
@Test
public void ChickenCategory_RegressionTest() throws Exception
{
	 int invalid=0;
...
Helper.customWait(8);
   ...
   chicken.click();
	...
	/*try {
      ...
      String chickenPageTitleExp="Licious | Chicken"	;
    
      String chickenPageTitleActual=driver.getTitle();
      Assert.assertEquals(chickenPageTitleActual, chickenPageTitleExp);
      etest.log(LogStatus.PASS,"Chicken Title is Same as expected");
      ...
      try {
		...
		//driver.findElement(By.xpath("(//div[@class='item-desc']/span[2])[1]")).click()
		  ...
		  System.out.println(addTocartByOne);
		   for (int i = 0; i < addToCart.size()-5; i++) {
			   try
				{
...
//			helper.highlightElement(driver, cart);
			Helper.safeJavaScriptClick(driver,cart);
			 ...
			 try
			 {
				 WebElement updatecart=driver.findElement(By.xpath("//button[contains(text(),'Update Cart')]"));
				 Helper.safeJavaScriptClick(driver,updatecart);
			 }catch(Exception e)
			 {
